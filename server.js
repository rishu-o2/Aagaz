...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (pathname.match(/^\/api\/admin\/tournaments\/[^/]+\/generate-fixtures$/) && req.method === 'POST') { if (!canManage(req, ['super_admin', 'tournament_manager'])) return send(res, 403, { error: 'Tournament manager access required.' }); const tournamentId = pathname.split('/')[4], data = await readData(), tournament = data.tournaments.find(item => item.id === tournamentId); if (!tournament) return send(res, 404, { error: 'Tournament not found.' }); const result = generateFixtures(data, tournament); if (result.error) return send(res, 409, { error: result.error }); data.events.push(...result.events); await writeData(data); return send(res, 201, { message: `${result.events.length} fixtures generated.`, events: result.events }); }

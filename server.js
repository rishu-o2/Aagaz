...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/auth/login' && req.method === 'POST') { const input = await body(req), data = readData(), user = data.staffUsers.find(item => item.email === String(input.email || '').trim().toLowerCase()); if (!user || !passwordMatches(input.password, user.passwordHash)) return send(res, 401, { error: 'Incorrect staff email or password.' }); const token = crypto.randomBytes(24).toString('hex'); sessions.set(token, { type: 'staff', role: user.role, userId: user.id, createdAt: Date.now() }); return send(res, 200, { token, user: { id: user.id, name: user.name, email: user.email, role: user.role } }); }
    if (pathname === '/api/admin/login' && req.method === 'POST') { const input = await body(req); if (input.password !== ADMIN_PASSWORD) return send(res, 401, { error: 'Incorrect password.' }); const token = crypto.randomBytes(24).toString('hex'); sessions.set(token, { type: 'staff', role: 'super_admin', userId: 'staff-admin', createdAt: Date.now() }); return send(res, 200, { token, user: { name: 'Aagaz Admin', role: 'super_admin' } }); }
    if (pathname === '/api/admin/data' && req.method === 'GET') { const session = staffSession(req); if (!session) return send(res, 401, { error: 'Sign in required.' }); const data = readData(); return send(res, 200, { ...data, user: session }); }

...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/teams/register' && req.method === 'POST') {
      ...
      const team = { id: id('team'), teamName: input.teamName.trim(), captainName: input.captainName.trim(), email: input.email.trim().toLowerCase(), passwordHash: hashPassword(input.password), createdAt: new Date().toISOString(), status: 'pending', members: [{ name: input.captainName.trim(), email: input.email.trim().toLowerCase(), role: 'Captain' }] }; data.teams.unshift(team); writeData(data); return send(res, 201, { message: 'Team created. Sign in to add your players.' });
    }
